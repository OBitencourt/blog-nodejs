...
exports.obter = async (req, res, next) => {
  try {
    const artigo = await Artigo.findById(req.params.id).populate(
      "autor",
      "nome email",
    );
...
exports.atualizar = async (req, res, next) => {
  try {
    ...
    if (
      artigo.autor.toString() !== req.utilizador._id.toString() &&
      req.utilizador.role !== "admin"
    ) {
      return res.status(403).json({ erro: "Sem permissão" });
    }
    const atualizado = await Artigo.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
      runValidators: true,
    });
